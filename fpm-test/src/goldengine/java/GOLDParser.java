...
public class GOLDParser implements GPMessageConstants, SymbolTypeConstants, ActionConstants, RecordIDConstants, ParseResultConstants
{
	...
	private Vector<FAState> pDFA = new Vector<FAState>(); //FAState
	private Vector<String> pCharacterSetTable = new Vector<String>(); //String

	//================================== Rules. Contains Rule Objects.
	private Vector<Rule> pRuleTable = new Vector<Rule>(); //Rule

	//================================== Grammar variables
	private VariableList pVariables = new VariableList();

	//================================== LALR(1) action table. Contains LRActionTables.
	private Vector<LRActionTable> pActionTable = new Vector<LRActionTable>(); //Contains LRStates
	...
	private Stack<Token> pTokens = new Stack<Token>(); //The set of tokens for 1. Expecting during error, 2. Reduction
	...
	private Stack<Token> pInputTokens = new Stack<Token>(); //Stack of tokens to be analyzed
	 ...
	 *             The engine parser should deal with
	 ...
	 * 
	 * @param theToken
	 *            A token to push onto the stack.
	 ***************************************************************/
	...
	public Token getToken(int index)
	{
		...
		{
			return pTokens.elementAt(index);
		}
		else
		{
			return null;
	 ...
	 * 
	 * @param value
	 *            The new Reduction to set as the current one.
	 ***************************************************************/
	public void setCurrentReduction(Reduction value)
	{
		if (pHaveReduction)
		{
			Token myTok = stack.peek();
			myTok.setData(value);
		}
	}
	...
	{
		if ((index >= 0) & (index < pSymbolTable.count())) { return pSymbolTable.getMember(index); }
	...
	{
		if ((index >= 0) & (index < pRuleTable.size())) { return pRuleTable.elementAt(index); }
	 ...
	 * 
	 * @param fileName
	 *            The absolute path of the .cgt file to load.
	 * @return True if the .cgt was loaded, false if not.
	 * @throws ParserException
	 *             If there was a stream access problem with
	 *             the file.
	 ***************************************************************/
	 ...
	 * 2. If parser is in comment mode then read tokens until a recognized one
	 * is found and report it<br>
	...
	private int parseToken(Token nextToken)
	{
		...
		LRActionTable lrAct = pActionTable.elementAt(currentLALR);
		...
		if (index != -1) //Work - shift or reduce
		{
			...
			switch (lrAct.item(index).actionConstant)
			{
				...
				case actionReduce:
					...
					currentRule = pRuleTable.elementAt(ruleIndex);
					...
					//Build a Reduction
					{
						...
						{
							newReduction.setToken(n, stack.pop());
						}
		...
		else
		{
			...
			lrAct = pActionTable.elementAt(currentLALR);
		...
		if (!source.done())
		{
			while (!done)
			{
				...
				else
				{
					...
					FAState faTmp = pDFA.elementAt(currentDFA);
					while ((n < faTmp.edgeCount()) & (!found))
					{
						charSetIndex = Integer.parseInt(faTmp.edge(n).getChars());
						String sT = pCharacterSetTable.elementAt(charSetIndex);
				...
				if (found)
				{
					...
					FAState faTmp2 = pDFA.elementAt(target);
				...
				//No edge found
				{
					...
					{
						FAState faTmp3 = pDFA.elementAt(lastAcceptState);
