 ...
 * 
 */
package it.freax.fpm.core.specs.tarball.antlr;
...
public class AntlrEngine
{
	...
	public AntlrEngine(String grammarName, String langName) throws ConfigurationReadException, ExtensionDecodingException, URISyntaxException, IOException
	{
		...
		File[] grammarsOnDisk = grammarsFolder.listFiles(new FileNameRegexFilter(".*\\.g"));
		List<String> grammarsOnJar = Arrays.asList(Streams.getResourceListing(getClass(), Constants.GRAMMARS_DIR, ".*\\.g"));
		if (!grammarsFolder.exists())
		{
			grammarsFolder.mkdirs();
			fillGrammarsFolder(grammarsFolder);
		}
		else if (grammarsOnDisk == null)
		{
			fillGrammarsFolder(grammarsFolder);
		}
		else if (!FpmCollections.getOne(grammarsOnDisk).diffOfStrings(grammarsOnJar, false).isEmpty())
		{
			fillGrammarsFolder(grammarsFolder);
		}
	...
	private void fillGrammarsFolder(File grammarsFolder)
	{
		try
		{
			...
			{
				files = Arrays.asList(grammarsFolder.listFiles(new FileNameRegexFilter(".*\\.g")));
				if (!FpmCollections.getOne(files).toStringsList().contains(grammar))
				{
