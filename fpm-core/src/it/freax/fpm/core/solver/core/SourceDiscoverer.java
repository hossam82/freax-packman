...
public class SourceDiscoverer extends Constants {
	...
	private Logger log = LogConfigurator.getOne(this.getClass()).configure(
			false);
	...
	public SourceDiscoverer(ArchiveReader reader, TarballSpec spec)
		...
		this.conf = Configuration.load(this.getSourceDiscoverConf());
	}
	...
	public TreeSet<Treenode> discover() throws IOException {
		...
		if (!this.reader.hasRead()) {
			start = this.time();
			this.reader.readEntries();
			this.log.debug("Read all entries: "
					+ TimeSpan.format(this.time() - start, TimeSpan.MILLISECOND));
		}
		this.log.debug("Inizializzo il treeset");
		ret = this.initTreeSet();

		start = this.time();

		Iterator<Treenode> it = ret.iterator();
		this.log.debug("Itero tutti i treenode");
		while (it.hasNext()) {
			Treenode tn = it.next();
			this.log.debug("Tento di riempire il nodo " + tn.getName());
			ArrayList<SrcFile> srcfiles = this.makeSrcFileList(tn,
					this.getFiles(tn.getName()));
			...
			this.log.debug("Dopo aver fatto tutto questo aggiorno la Compilation Unit che ho appena settato sul treenode padre");
			tn.updateCU(cu);
			this.log.debug("Emerge che il nome della Compilation Unit e la sua versione sono:");
			this.log.debug(tn.getAssociatedCU().getDummySpec().getPackage()
					.getPackageName());
			this.log.debug(tn.getAssociatedCU().getDummySpec().getPackage()
					.getVersion());
		}
		this.log.debug("Una volta processati tutti i nodi setto il DummySpec");
		this.spec.map(this.getGlobalDummySpec(ret));
		this.log.debug(this.spec);
		this.log.debug("Tempo impiegato: "
				+ TimeSpan.format(this.time() - start, TimeSpan.MILLISECOND));
		return ret;
	}
	...
	private void makeInstructionSet(Treenode tn, ArrayList<SrcFile> files) {
		...
		for (SrcFile file : files)
			for (String lang : file.getLangs()) {
				this.log.debug("Cicli annidati: Per ogni file su tutti i linguaggi associati for (files) { for (files.linguaggi) { \""
						+ lang + "\" } }");
				...
				if (file.isNotable()) {
					...
					instructions.addAll(type.getInstructionsById(notable
							.getId()));
					this.log.debug("Dato che è notevole prendo l'instruction set "
							+ notable.getId() + " dalla configurazione");
				} else {
					this.log.debug("Dato che non è notevole prendo l'instruction set 0 dalla configurazione");
					instructions.addAll(type.getInstructionsById(0));
				}
				this.log.debug("A prescindere prendo l'instruction set -1 dalla configurazione");
		...
		this.log.debug("Aggiungo i comandi di Privilege Excalation quando necessari nell'instruction set");
		while (it.hasNext()) {
		...
		this.log.debug("Aggiungo l'istruzione alla Compilation Unit facendo un merge dinamico di tutto l'instruction set");
	...
	private ArrayList<SrcFile> makeSrcFileList(Treenode tn,
		...
		this.log.debug("Costruisco la lista di SrcFile");
		...
		for (String file : files) {
			...
			if (srcfile.isNotable() || srcfile.getLangs().size() > 0) {
				this.log.debug("Il file è buono quindi setto la Compilation Unit");
				tn.setIsCU(true);
	...
	private ArrayList<SrcFile> cleanSrcFiles(ArrayList<SrcFile> srcfiles) {
		this.log.debug("Pulisco la lista di SrcFiles dagli oggetti inutili (non-notevoli e non-linguaggi)");
	...
	private SrcFile makeSrcFile(String file) {
		this.log.debug("Creo e valorizzo il SrcFile con nome \"" + file + "\"");
		SrcFile srcfile = new SrcFile();
		srcfile.setName(file);
		srcfile.setNotable(this.isFileParticolare(file));
		this.log.debug("Il file" + (srcfile.isNotable() ? " " : " non ")
				+ "è notevole");
		String content = this.reader.getEntryContent(file);
		this.log.debug("Setto il contenuto del file");
		srcfile.setContent(content);
		if (srcfile.isNotable()) {
			this.log.debug("Dato che è notevole trovo tutti i linguaggi per i quali è notevole");
			srcfile.addAllLangs(this.getNotableFileLangs(file));
			this.log.debug("che sono:\n" + srcfile.getLangs());
		} else {
			this.log.debug("Dato che non è notevole devo usare altri metodi per verificare che file è");
			Iterator<ConfType> typeit = this.conf.typesIterator();
			EbnfParser parser = getEbnfParser(null, null);
			while (typeit.hasNext()) {
				ConfType type = typeit.next();
				this.log.debug("Provo col tipo " + type.getSource());
				try {
					if (Strings.getOne().checkExtensions(file, type.getExts())) {
						this.log.debug("Il file ricade nelle estensioni del tipo, quindi posso provare il parsing con l'ebnf "
								+ type.getEbnf());
						parser = getEbnfParser(parser, type.getEbnf());
						if (parser.parse(content)) {
							this.log.debug("Sono riuscito a parsare il contenuto! Il file è di tipo \""
									+ type.getSource()
					...
					ErrorHandler.getOne(this.getClass()).handle(e);
				}
			}
		}
		return srcfile;
	...
	private void setCompilationUnit(CompilationUnit cu, ArrayList<SrcFile> files) {
		this.log.debug("Setto la Compilation Unit con " + files.size()
				+ " SrcFile(s), di questi scelgo solo quelli notevoli");
		for (SrcFile file : files)
			if (file.isNotable())
				for (String lang : file.getLangs()) {
					this.log.debug("Controllo se il file notevole contiene informazioni additive nel linguaggio "
							+ lang);
					ConfType type = this.conf.getType(lang);
					...
					this.log.debug("A questo punto devo fare una cosa particolare: dato che gli additives sono categorizzati, devo intanto dividerli per categoria");
					Iterator<Entry<InfoType, ArrayList<Additive>>> it;
					it = this.getTypedAdditives(additives).entrySet()
							.iterator();
					while (it.hasNext()) {
						Entry<InfoType, ArrayList<Additive>> elem = it.next();
						this.log.debug("Poi per ogni tipo di additive prendo tutti quelli della categoria "
								+ elem.getKey()
								+ " e trovo quale risulta essere il migliore");
						String best = this.getBestScore(elem.getValue(), file);
						this.log.debug("La parte importante arriva qui: setto il payload nell'attributo specifico a seconda del tipo di additive che ho processato");
						switch (elem.getKey()) {
						case PackageName: {
							this.log.debug("Setto " + elem.getKey() + " come "
									+ best);
						...
						case Changelog: {
							this.log.debug("Setto " + elem.getKey());
	...
	private String getBestScore(ArrayList<Additive> additives, SrcFile file) {
		String ret = "";
		this.log.debug("Istanzio un EntriesScorer, cioè un oggetto che sa trovare il più (o meno) presente tra gli elementi che gli vengono passati");
		EntriesScorer<String> es = new EntriesScorer<String>(true);
		this.log.debug("L'algoritmo lavora in modo da ottenere l'output del metodo che l'additive deve eseguire sul file per trovare il suo payload");
		this.log.debug("Poi il payload viene inserito nell'EntriesScorer che analizza quale payload sia più presente, e lo segnala come più plausibile");
	...
	private HashMap<InfoType, ArrayList<Additive>> getTypedAdditives(
		...
		this.log.debug("Riempio un HashMap<InfoType, ArrayList<Additive>> che divida i set di Additive in categorie");
		for (Additive additive : additives) {
	...
	private DummySpec getGlobalDummySpec(TreeSet<Treenode> set) {
		...
		this.log.warn("PORK AROUND!!!");
		this.log.debug("Ciclo nuovamente il treeset, sta volta controllo che il treenode sia una CU, visto che ho trovato il dato");
		this.log.debug("Se è una CU prendo il suo DummySpec e da quello prendo il packageName e la versione");
		this.log.debug("Passo questi dati a due EntriesScorer settati apposta per poter capire qual'è il nome del software, tra tutte le CU");
		while (it.hasNext()) {
			Treenode tn = it.next();
			if (tn.isCU()) {
		...
		this.log.debug("E in fine setto il best dei due EntriesScorer");
